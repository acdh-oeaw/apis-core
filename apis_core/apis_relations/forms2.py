...
class GenericRelationForm(forms.ModelForm):
    ...
    def get_html_table(self, entity_type, request, site_instance, form_match):
        table = get_generic_relations_table(
            relation_class=self.relation_form, entity_instance=site_instance, detail=False
        )
        ...
        if form_match.group(1) == form_match.group(2):
            ...
            if "apis_highlighter" in settings.INSTALLED_APPS:
                objects = self.relation_form.objects.filter_ann_proj(request=request).filter(Q(**dic_a) | Q(**dic_b))
            else:
            ...
            if "apis_highlighter" in settings.INSTALLED_APPS:
                ttab = self.relation_form.objects.filter_ann_proj(request=request).filter(**tab_query)
            else:
    ...
    def __init__(self, siteID=None, highlighter=False, *args, **kwargs):
        ...
        if lst_src_target[0] == lst_src_target[1]:
            ...
            self.fields["relation_type"] = autocomplete.Select2ListCreateChoiceField(
                ...
                widget=ListSelect2(
                    ...
                    url=reverse(
                        "apis:apis_vocabularies:generic_vocabularies_autocomplete",
                        args=["".join([lst_src_target[0].lower(), lst_src_target[1].lower(), "relation"]), "reverse"],
                    ),
                    attrs=attrs,
                ),
            )
        ...
        if instance and instance.id:
            ...
            if self.rel_accessor[1]:
                self.fields["relation_type"].choices = [(instance.relation_type.id, instance.relation_type.label)]
