...
class GenericRelationForm(forms.ModelForm):
    ...
    def __init__(self, siteID=None, highlighter=False, *args, **kwargs):
        ...
        if (
            "instance" in kwargs
            and hasattr(kwargs["instance"], "bible_ref_url")
            and (
                kwargs["instance"].relation_type_id == 204
                or kwargs["instance"].relation_type_id == 205
            )
        ):
            bible_ref_url = kwargs["instance"].bible_ref_url
            bible_ref_label = kwargs["instance"].bible_ref_label
            if bible_ref_url is not None and bible_ref_label is not None:
                self.fields['references'].help_text =  f"<a href={bible_ref_url}>{bible_ref_label}</a>"
