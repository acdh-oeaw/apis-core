...
class RelationPublishedQueryset(models.QuerySet):
    ...
    def filter_ann_proj(self, request=None, ann_proj=1, include_all=True):
        ...
        # NEW:
        query = Q(annotation_set_new__annotation_project_id=ann_proj)
        if users_show is not None:
            query.add(Q(annotation_set_new__user_added_id__in=users_show), Q.AND)
        if include_all:
            query.add(Q(annotation_set_new__annotation_project__isnull=True), Q.OR)
