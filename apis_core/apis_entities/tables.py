...
def get_entities_table_ORIGINAL(entity, edit_v, default_cols):

    if default_cols is None:
        default_cols = ['name', ]

    # TODO __sresch__ : consider moving this class outside the function (if not restricted by need for meta entity class)
    class GenericEntitiesTable(tables.Table):
        if edit_v:
            name = tables.LinkColumn(
                'apis:apis_entities:generic_entities_edit_view',
